  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={handleSave} className="flex items-center gap-2 bg-[#002045] text-white px-4 py-2 rounded-lg text-sm font-semibold hover:opacity-90 shadow-sm">
        ...
        <div className="flex-1 space-y-6">
          ...
          {activeSection === "clinic" && (
            <SettingsCard title="Clinic Profile" desc="Basic information displayed on invoices, reports, and patient communications">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <Field label="Clinic Name"><input className="input-field" defaultValue="" placeholder="Al-Shifa Medical Center" /></Field>
                <Field label="License Number"><input className="input-field" defaultValue="" placeholder="MH-2024-XXXXX" /></Field>
                <Field label="Phone"><input className="input-field" defaultValue="" placeholder="+962 6 000 0000" /></Field>
                <Field label="Email"><input type="email" className="input-field" defaultValue="" placeholder="info@clinic.com" /></Field>
                <Field label="Address" full><textarea className="input-field resize-none" rows={2} defaultValue="" placeholder="Street address, city, country" /></Field>
                <Field label="Tax ID"><input className="input-field" defaultValue="" placeholder="TIN number" /></Field>
                <Field label="Currency"><select className="input-field"><option value="USD">USD — US Dollar</option><option value="JOD">JOD — Jordanian Dinar</option><option value="SAR">SAR — Saudi Riyal</option></select></Field>
                <Field label="Time Zone"><select className="input-field"><option>Asia/Amman (GMT+3)</option><option>Asia/Riyadh (GMT+3)</option><option>UTC</option></select></Field>
              </div>
            </SettingsCard>
          )}
          ...
          {activeSection === "security" && (
            <SettingsCard title="Security & Authentication" desc="Password policies, session management, and access controls">
              <div className="space-y-5">
                <Toggle label="Two-Factor Authentication (2FA)" desc="Require 2FA for all users on login" defaultChecked />
                <Toggle label="Account Lockout" desc="Lock account after 5 failed login attempts for 15 minutes" defaultChecked />
                <Toggle label="Single Sign-On (SSO)" desc="Enable SSO via SAML 2.0 or OAuth 2.0" />
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5 pt-2">
                  <Field label="Session Timeout (minutes)"><input type="number" className="input-field" defaultValue={15} min={5} max={480} /></Field>
                  <Field label="Password Min Length"><input type="number" className="input-field" defaultValue={8} min={8} max={32} /></Field>
                  <Field label="Password Expiry (days)"><input type="number" className="input-field" defaultValue={90} min={30} max={365} /></Field>
                  <Field label="Failed Login Limit"><input type="number" className="input-field" defaultValue={5} min={3} max={10} /></Field>
                </div>
              </div>
            </SettingsCard>
          )}
            ...
            <SettingsCard title="Notifications & Alerts" desc="Configure how and when the system sends notifications">
              <div className="space-y-5">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <Field label="Email Provider"><select className="input-field"><option value="">SMTP</option><option>SendGrid</option><option>Mailgun</option></select></Field>
          ...
          {activeSection === "billing" && (
            <SettingsCard title="Billing Configuration" desc="Tax rates, invoice numbering, and payment settings">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <Field label="Default Tax Rate (%)"><input type="number" className="input-field" defaultValue={7} min={0} max={100} step={0.5} /></Field>
                <Field label="Invoice Prefix"><input className="input-field" defaultValue="INV" placeholder="INV" /></Field>
                <Field label="Payment Terms (days)"><input type="number" className="input-field" defaultValue={30} /></Field>
                <Field label="Default Discount Authorization">
                  <select className="input-field"><option>Receptionist (up to 10%)</option><option>Manager (up to 50%)</option><option>No restriction</option></select>
                </Field>
              </div>
          ...
          {activeSection === "integrations" && (
            <div className="space-y-4">
              {[
                { icon: "sms",         label: "SMS Gateway",       desc: "Twilio / local SMS provider for patient alerts",   status: "Not configured", statusColor: "text-[#d97706]" },
                { icon: "email",       label: "Email Server",      desc: "SMTP or SendGrid for email notifications",         status: "Not configured", statusColor: "text-[#d97706]" },
                { icon: "payments",    label: "Payment Gateway",   desc: "Stripe or local payment processor",                 status: "Not configured", statusColor: "text-[#d97706]" },
                { icon: "health_and_safety", label: "Insurance Portal", desc: "HL7 / REST integration with insurance providers", status: "Phase 2", statusColor: "text-[#74777f]" },
                { icon: "biotech",     label: "Lab Analyzers",     desc: "HL7 v2.x / FHIR interface with lab equipment",     status: "Phase 2", statusColor: "text-[#74777f]" },
                { icon: "radiology",   label: "PACS System",       desc: "DICOM integration for radiology images",            status: "Phase 3", statusColor: "text-[#74777f]" },
              ].map((intg) => (
                <div key={intg.label} className="bg-white rounded-xl border border-[#e3e2e6] p-5 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    <span className={`text-xs font-semibold ${intg.statusColor}`}>{intg.status}</span>
  ...
  return (
    <div className="flex items-center justify-between py-2">
      ...
      <button
        ...
        className={`relative w-11 h-6 rounded-full transition-colors ${checked ? "bg-[#002045]" : "bg-[#c4c6cf]"}`}
      >
        <span className={`absolute top-0.5 w-5 h-5 rounded-full bg-white shadow transition-transform ${checked ? "translate-x-5" : "translate-x-0.5"}`}></span>
      </button>
    </div>
  );
}
