...
export default function DashboardClient({ stats, appointments }: Props) {
  ...
  return (
    <div className="space-y-8">
      ...
      >
        <div>
          <h1 className="text-2xl font-bold text-[var(--txt1)]">{d.title}</h1>
          <p className="text-sm text-[var(--txt3)] mt-0.5">{dateStr}</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="hidden md:flex items-center gap-1.5 text-xs text-[var(--txt3)]">
            <span className={`w-2 h-2 rounded-full ${sseStatus === "connected" ? "bg-[#0d9488] animate-pulse" : "bg-[var(--txt4)]"}`} aria-hidden="true" />
        ...
        {kpiCards.map((card) => (
          ...
          >
            <div className="flex justify-between items-start mb-4">
              ...
              {card.change != null && (
                <div className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold ${card.change >= 0 ? "bg-[#ccfbf1]/70 text-[#0d9488]" : "bg-[#ffdad6] text-[#93000a]"}`}>
            ...
            <p className="text-xs font-semibold text-[var(--txt3)] uppercase tracking-wider mb-1">{card.label}</p>
      ...
      {stats && (stats.criticalAlerts > 0 || stats.lowStockItems > 0 || stats.pendingInvoices > 0) && (
        ...
        >
          {stats.criticalAlerts > 0 && (
            <AlertBanner icon="emergency" color="text-[#ba1a1a]" bg="bg-[#ffdad6]" border="border-[#ba1a1a]/20"
              label={d.criticalAlerts} value={stats.criticalAlerts} link="/reports" linkLabel={d.reviewNow} />
          )}
          {stats.lowStockItems > 0 && (
            <AlertBanner icon="inventory_2" color="text-[#d97706]" bg="bg-[#fff7ed]" border="border-[#d97706]/20"
              label={d.lowStock} value={stats.lowStockItems} link="/pharmacy" linkLabel={d.viewPharmacy} />
          )}
          {stats.pendingInvoices > 0 && (
            <AlertBanner icon="receipt_long" color="text-[#1960a3]" bg="bg-[#eff6ff]" border="border-[#1960a3]/20"
      ...
      >
        ...
        <div className="lg:col-span-2 card p-6 flex flex-col">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h3 className="text-lg font-semibold text-[var(--txt1)]">{d.revenueTitle}</h3>
              <p className="text-xs text-[var(--txt3)]">{d.revenueSubtitle}</p>
            </div>
            ...
            {revenue.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={revenue} barCategoryGap="30%" barGap={4}>
                  ...
                  <XAxis dataKey="month" tick={{ fontSize: 12, fill: "var(--txt3)" }} axisLine={false} tickLine={false} />
                  <YAxis tick={{ fontSize: 11, fill: "var(--txt3)" }} axisLine={false} tickLine={false}
                    tickFormatter={(v) => v >= 1000 ? `${(v / 1000).toFixed(0)}k` : v} />
                  <Tooltip
                    contentStyle={{ border: "1px solid var(--border)", borderRadius: 8, fontSize: 12, backgroundColor: "var(--surface)" }}
                    labelStyle={{ color: "var(--txt1)" }}
              ...
              <div className="h-full flex flex-col items-center justify-center gap-2 border border-[var(--border)] rounded-lg bg-[var(--surface2)]">
                <span className="material-symbols-outlined text-[var(--txt4)] text-4xl">bar_chart</span>
                <p className="text-xs text-[var(--txt3)] text-center">{d.loadingRevenue}</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="card p-6">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-lg font-semibold text-[var(--txt1)]">{d.deptLoad}</h3>
          ...
          {depts.length > 0 ? (
              ...
              {depts.map((dept) => (
                <div key={dept.department}>
                  <div className="flex justify-between text-xs mb-1">
                    <span className="font-medium text-[var(--txt1)] truncate max-w-[140px]">{dept.department}</span>
                    <span className="text-[var(--txt3)] font-semibold">{dept.count} {d.appts}</span>
                  </div>
                  <div className="w-full bg-[var(--surface3)] rounded-full h-2">
                    <motion.div
            ...
            <div className="flex flex-col items-center justify-center gap-2 py-12">
              <span className="material-symbols-outlined text-[var(--txt4)] text-4xl">business</span>
              <p className="text-xs text-[var(--txt3)] text-center">
      ...
      >
        ...
        <div className="lg:col-span-2 card overflow-hidden">
          <div className="p-5 border-b border-[var(--border)] flex justify-between items-center">
            <div className="flex items-center gap-3">
              <h3 className="text-lg font-semibold text-[var(--txt1)]">{d.todaySchedule}</h3>
            ...
            {appointments.length === 0 ? (
              <div className="py-16 flex flex-col items-center gap-3">
                <div className="w-14 h-14 bg-[var(--surface2)] rounded-2xl flex items-center justify-center">
                  <span className="material-symbols-outlined text-[var(--txt3)] text-2xl">calendar_today</span>
                </div>
                <p className="text-sm text-[var(--txt3)] font-medium">{d.noAppointments}</p>
                ...
                return (
                  ...
                  >
                    <Link
                      ...
                      className={`flex items-center justify-between p-4 hover:bg-[var(--surface2)] transition-colors group ${isUrgent ? "bg-[#fff5f5]" : ""}`}
                    >
                      <div className="flex items-center gap-4">
                        <div className={`w-14 h-14 flex flex-col items-center justify-center rounded-xl text-center ${isUrgent ? "bg-[#ffdad6] text-[#ba1a1a]" : "bg-[var(--surface2)] text-[var(--txt2)]"}`}>
                        ...
                        <div>
                          <p className="text-sm font-semibold text-[var(--txt1)] group-hover:text-[#1960a3] transition-colors">
                            {apt.patientName}
                          </p>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <span className="material-symbols-outlined text-[18px] text-[var(--txt3)] group-hover:text-[#1960a3]">
                          chevron_right
                        </span>
                      </div>
                    </Link>
        ...
        <div className="space-y-4">
          <div className="card p-5">
            <h3 className="text-sm font-semibold text-[var(--txt3)] uppercase tracking-wider mb-4">{d.quickLinks}</h3>
            <div className="grid grid-cols-2 gap-3">
              {quickLinks.map((q) => (
                <Link
                  key={q.href}
                  href={q.href}
                  className="flex flex-col items-center gap-2 p-4 bg-[var(--surface2)] rounded-xl hover:bg-[var(--surface3)] transition-colors text-center group"
                >
                  <span className={`material-symbols-outlined text-[24px] ${q.color} group-hover:scale-110 transition-transform`}>
                    {q.icon}
                  </span>
                  <span className="text-xs font-semibold text-[var(--txt1)] leading-tight">{q.label}</span>
                </Link>
              ))}
            </div>
          </div>
          ...
          <div className="card p-5">
            <h3 className="text-sm font-semibold text-[var(--txt3)] uppercase tracking-wider mb-4">{d.systemStatus}</h3>
            <div className="space-y-3">
              {systemItems.map((item) => (
                <div key={item.label} className="flex items-center justify-between">
                  <span className="text-sm text-[var(--txt1)]">{item.label}</span>
                  <span className={`flex items-center gap-1.5 text-xs font-semibold ${item.ok ? "text-[#0d9488]" : "text-[#d97706]"}`}>
                    <span className={`w-2 h-2 rounded-full flex-shrink-0 ${item.ok ? "bg-[#0d9488]" : "bg-[#d97706] animate-pulse"}`} />
                    {item.status}
                  </span>
                </div>
              ))}
  ...
  return (
    <div className={`${bg} border ${border} rounded-xl p-4 flex items-center justify-between`}>
      <div className="flex items-center gap-3">
        ...
        <p className="text-sm font-semibold text-[var(--txt1)]">{value} {label}</p>
      </div>
