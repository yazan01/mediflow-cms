...
def list_branches(db: Session = Depends(get_db), _user=Depends(get_current_user)):
    ...
    return [branch_to_dict(b, db) for b in branches]


@router.post("", status_code=201)
def create_branch(body: BranchCreate, db: Session = Depends(get_db), user=Depends(get_current_user)):
...
def update_branch(branch_id: str, body: BranchUpdate, db: Session = Depends(get_db), user=Depends(get_current_user)):
    branch = db.query(models.Branch).filter(models.Branch.id == branch_id).first()
    if not branch:
        raise HTTPException(404, "Branch not found")
    if body.code:
