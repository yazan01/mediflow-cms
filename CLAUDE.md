...
- **Security headers**: CSP, X-Frame-Options, X-Content-Type-Options, Referrer-Policy set in `next.config.ts`
...
- **Reset DB**: `cd backend && python reset_db.py --confirm` — requires `--confirm` flag (3-second abort window)
